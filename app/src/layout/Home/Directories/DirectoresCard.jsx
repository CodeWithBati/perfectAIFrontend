...
import { createDirectorySlugName, truncateWithEllipsis } from "@/lib/form";
...
import StarsRating from "@/app/src/ui/StarsRating";
import { toastText } from "@/constants/text-constants";

const DirectoresCard = ({ dir }) => {
  const router = useRouter();
  const dirSlugName = createDirectorySlugName(dir.name);
  ...
  const sentToDirectoryView = () => {
    router.push(`/directories/${dir.slug}`);
  };

  return (
    <div onClick={sentToDirectoryView} className=" cursor-pointer">
          ...
          <div className=" mt-4">
            <p className=" text-slate-600 dark:text-slate-300">
              {truncateWithEllipsis(
                dir.summary || dir.description.replace(/<[^>]+>/g, ""),
                200
              )}
            </p>
          </div>
