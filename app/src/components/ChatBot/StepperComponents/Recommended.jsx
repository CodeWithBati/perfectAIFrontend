...
const Recommended = ({ data }) => {
  ...
  return (
    data.directory ?
    <div className='max-w-96 bg-white dark:bg-slate-950 p-5 rounded-md shadow-md'>
      <div className='py-3'>
        <div className=' flex gap-2 items-center'>
          <div className="w-12 h-12 md:w-14 md:h-14 flex justify-center items-center border border-gray-300 dark:border-gray-800 rounded-lg relative">
            <Image
              ...
              height={100}
              alt="Directory Icon"
              src={data.directory.icon}
              className=" p-2"
            />
          </div>
          <div>
            <h1 className=' text-xl font-medium text-black dark:text-white'>{data.directory.name}</h1>
          </div>
        </div>
      </div>
      <p className='text-black dark:text-white my-2'>{data.description}</p>
      <div className='w-40 ml-auto'>
        <div
          onClick={()=>handleRedirectToDirectoryPage(data.directory.slug)}
    ...
    <div className='max-w-96 bg-white dark:bg-slate-950 p-5 rounded-md shadow-md'>
      <p className='dark:text-white text-black'>This AI tool has been removed from the directory</p>
    </div>
  )
}
