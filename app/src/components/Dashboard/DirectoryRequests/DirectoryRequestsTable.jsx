...
const DirectoryRequestsTable = () => {
  ...
  const fetchData = useCallback(
    async (page, searchQuery = "", limit = 10) => {
      let url = `${process.env.NEXT_PUBLIC_API_URL}/directories/requests?page=${page}&limit=${limit}`;
  ...
  const columns = [
    ...
    columnHelper.display({
      id: "tableAction",
      header: (info) => <div className=" text-center">Actions</div>,
      cell: (info) => {
        return (
          <>
  ...
  return (
    <>
      <DataTable
        ...
        setLimit={setLimit}
        limit={limit}
        noDataMessage="All Directory Requests are Approved"
      />
    </>
  );
};

export default DirectoryRequestsTable;
