...
import { cleanObject } from "@/lib/form";
...
function ApproveDirectoryForm({
  ...
  const handleInputChange = (field, value) => {
    setApproveFormData((prevFormData) => {
      return {
        ...prevFormData,
        [field]: value,
      };
  ...
  const handleSubmit = async () => {
    setSpinner(true);
    const cleanedapproveFormData = cleanObject(approveFormData);

    try {
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/directories/${formData.id}/approve`,
        cleanedapproveFormData,
        { headers: { Authorization: "Bearer " + token } }
      );
      if (response.status === 200) {
        toast.success(toastText.success.directoryApproved);
        router.push("/dashboard/directories");
      } else {
        toast.error(toastText.error.directoryNotApproved);
      }
    } catch (error) {
      console.log(error);
      toast.error(toastText.error.directoryNotApproved);
              ...
              <div className="flex flex-wrap -mx-3 -my-2">
                <div className="w-full lg:w-1/2 px-3">
                  <div className="py-2 flex gap-1 flex-col">
                    <Label htmlFor="Upload Icon" className="mb-2">
                      Upload Icon
                    </Label>
                      ...
                      <div className=" flex flex-row gap-4">
                        ...
                        <div className="py-2">
                          <Switch
                            label="Featured"
                            size="sm"
                            checked={approveFormData.isFeatured}
                            onChange={() =>
                              handleInputChange(
                                "isFeatured",
                                !approveFormData.isFeatured
                              )
                            }
                          />
                        </div>
...
export default ApproveDirectoryForm;
